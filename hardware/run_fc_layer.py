...
# Apps to benchmark — both fp16 and fp32 batch-12 variants
SIM_APPS = ['fc_layer16_batch12', 'fc_layer32_batch12']

# Per-app, per-run output file name template
OUTPUT_CSV_TEMPLATE = "ara_{app}_l{lanes}_vlen{vlen_bytes}.csv"

# Simulation parameters
LANES_LIST = [4]
VLEN_BYTES_LIST = [8192]
...
def main():
    ...
    print("========================================")
    print(f"Testing applications: {', '.join(SIM_APPS)}")
    print("========================================\n")
    ...
    for lanes in LANES_LIST:
        for vlen_bytes in VLEN_BYTES_LIST:
            ...
            # 1. Verilate once per (lanes, vlen) configuration
            run_command(f"make verilate config={TEMP_CONFIG_NAME}")

            # 2. Run each app against the same verilated model
            for sim_app in SIM_APPS:
                output_csv = os.path.join(
                    BASE_DIR,
                    OUTPUT_CSV_TEMPLATE.format(app=sim_app, lanes=lanes, vlen_bytes=vlen_bytes)
                )
