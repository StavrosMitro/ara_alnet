...
static void mp_op_forward_single(void *argv)
{
    ...
    register int *indices = mp.op->max_indices + mp.batch_id * mp.op->out_units;
    ...
    for (register int c = 0; c < channels; c++)
    {
        for (int j = 0; j < mp.op->in_h-strides+1; j += strides) {
            ...
            int i = 0;                   
            int o_x = 0;     
            register unsigned long byte_stride = 2 * sizeof(float);            

            while (o_w_left > 0) {
                unsigned long int vl;
                asm volatile("vsetvli %0, %1, e32, m4, ta, ma" : "=r"(vl) : "r"(o_w_left));

                float *row0 = &input[i + j * mp.op->in_w + c * iwih];
                float *row1 = row0 + mp.op->in_w;
                int *idx_ptr = &indices[o_x + (j/2) * mp.op->out_w + c * owoh]; 
                float *out_ptr = &output[o_x + (j/2) * mp.op->out_w + c * owoh];

                asm volatile("vlse32.v v4,  (%0), %1" : : "r"(row0),     "r"(byte_stride)); // P0
                asm volatile("vlse32.v v8,  (%0), %1" : : "r"(row0 + 1), "r"(byte_stride)); // P1

                asm volatile("vlse32.v v12, (%0), %1" : : "r"(row1),     "r"(byte_stride)); // P2
                asm volatile("vlse32.v v16, (%0), %1" : : "r"(row1 + 1), "r"(byte_stride)); // P3
                ...
                asm volatile("vse32.v v28, (%0)" : : "r"(out_ptr));
                ...
                asm volatile("vse32.v v20, (%0)" : : "r"(idx_ptr));
...
static void mp_op_backward_single(void *argv)
{
    ...
    register float *d_input  = mp.op->d_input + mp.batch_id * mp.op->in_units;
    register float *d_output = mp.op->d_output + mp.batch_id * mp.op->out_units;
    register int   *indices  = mp.op->max_indices + mp.batch_id * mp.op->out_units;
    ...
    for (register int c = 0; c < channels; c++)
    {
        for (int j = 0; j < in_h - strides + 1; j += strides) {
            ...
            while (o_w_left > 0) {
                ...
                float *row0 = &d_input[i + j * in_w + c * iwih];
                float *row1 = row0 + in_w;
                
                float *d_out_ptr = &d_output[o_x + (j/2) * out_w + c * owoh];
                int   *idx_ptr   = &indices[o_x + (j/2) * out_w + c * owoh];
                ...
                asm volatile(
                    ...
                    "vmseq.vi v0, v20, 0 \n\t"
                    "vsse32.v v28, (%2), %6, v0.t \n\t"
                    
                    "vmseq.vi v0, v20, 1 \n\t"
                    "vsse32.v v28, (%3), %6, v0.t \n\t"
                    
                    "vmseq.vi v0, v20, 2 \n\t"
                    "vsse32.v v28, (%4), %6, v0.t \n\t"
                    
                    "vmseq.vi v0, v20, 3 \n\t"
                    "vsse32.v v28, (%5), %6, v0.t"
                    :
...
static void mp_audit_indices(max_pooling_op *op)
{
    ...
    for (int i = 0; i < out_w; i++) {
        int x0 = i * ps, y0 = j * ps;
        float best = -1111111.0f; int bx = 0, by = 0, neq = 0;
        for (int x = x0; x < MIN(x0 + ps, in_w); x++)
            for (int y = y0; y < MIN(y0 + ps, in_h); y++) {
                float v = op->input[p*ch*iwih + c*iwih + y*in_w + x];
