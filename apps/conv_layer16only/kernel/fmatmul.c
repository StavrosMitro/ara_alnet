...
void fmatmul(_Float16 *c, const _Float16 *a, const _Float16 *b,
...
void fmatmul_nt(_Float16 *c, const _Float16 *a, const _Float16 *b,
                const unsigned long int M, const unsigned long int N,
                const unsigned long int P) {
  if (M <= 4) {
...
void fmatmul_tn(_Float16 *c, const _Float16 *a, const _Float16 *b,
                const unsigned long int M, const unsigned long int N,
                const unsigned long int P) {
  if (N <= 4) {
...
void fmatmul_4x4_tn(_Float16 *c, const _Float16 *a, const _Float16 *b,
  ...
  for (unsigned long int p = 0; p < P; p += block_size_p) {
    ...
    for (unsigned long int n = 0; n < N; n += block_size) {
      ...
      fmatmul_vec_4x4_slice_init();
      fmatmul_vec_4x4_tn(c__, a_, b_, M, P, N);
    }
  }
}
...
static inline void fmatmul_vec_4x4_deferred(_Float16 *c, const _Float16 *a,
                                            const _Float16 *b,
  ...
  asm volatile("vsetvli zero, %0, e16, m1, ta, ma" : : "r"(N));
  ...
  while (k > 0) {
    ...
    asm volatile("vsetvli %0, %1, e16, m1, ta, ma" : "=r"(vl) : "r"(k));

    asm volatile("vle16.v v0, (%0)" : : "r"(a0));
    a0 += vl;
    asm volatile("vle16.v v1, (%0)" : : "r"(a1));
    a1 += vl;
    asm volatile("vle16.v v2, (%0)" : : "r"(a2));
    a2 += vl;
    asm volatile("vle16.v v3, (%0)" : : "r"(a3));
    a3 += vl;

    asm volatile("vle16.v v4, (%0)" : : "r"(b0));
    b0 += vl;
    asm volatile("vle16.v v5, (%0)" : : "r"(b1));
    b1 += vl;
    asm volatile("vle16.v v6, (%0)" : : "r"(b2));
    b2 += vl;
    asm volatile("vle16.v v7, (%0)" : : "r"(b3));
    b3 += vl;
...
void fmatmul_8x8_tn(_Float16 *c, const _Float16 *a, const _Float16 *b,
  ...
  for (unsigned long int p = 0; p < P; p += block_size_p) {
    ...
    for (unsigned long int n = 0; n < N; n += block_size) {
      ...
      fmatmul_vec_8x8_slice_init();
      fmatmul_vec_8x8_tn(c__, a_, b_, M, P, N);
    }
  }
}
...
void fmatmul_16x16_tn(_Float16 *c, const _Float16 *a, const _Float16 *b,
  ...
  for (unsigned long int p = 0; p < P; p += block_size_p) {
    ...
    for (unsigned long int n = 0; n < N; n += block_size) {
      ...
      fmatmul_vec_16x16_slice_init();
      fmatmul_vec_16x16_tn(c__, a_, b_, M, P, N);
    }
  }
}
