...
_Float16 shared_memory_pool[MATRIX_TRANSPOSE_WORKSPACE_ELEMS];

// _Float16 fmatmul_a_scratch[FMATMUL_MAX_M * FMATMUL_MAX_N];
_Float16 fmatmul_c_scratch[FMATMUL_MAX_M * FMATMUL_MAX_K];
                        ...
                        const int M, const int N, const int K)
{
    ...
    _Float16 *a_pad = shared_memory_pool;
    for (int m = 0; m < M; m++)
    {
                              ...
                              const _Float16 eps)
{
    ...
    _Float16 *tn_out = shared_memory_pool;
    ...
    for (int n = 0; n < N; n++)
    {
        for (int k = 0; k < K; k++)
        {
            ...
            _Float16 diff = fabsf(sum - tn_out[(size_t)n * (size_t)K + (size_t)k]);
            if (diff > max_diff)
                max_diff = diff;
            if (diff > eps)
                mismatch_count++;
                              ...
                              const _Float16 eps)
{
    ...
    _Float16 max_diff = 0.0f;
    for (size_t idx = 0; idx < mk; idx++) {
        _Float16 diff = fabsf(fmatmul_nt_loop_scratch[idx] - fmatmul_nt_out_scratch[idx]);
        if (diff > max_diff)
            max_diff = diff;
        if (diff > eps)
            mismatch_count++;
...
void matrix_transpose(_Float16 *x, int m, int n)
{
    ...
    _Float16 *tmp = shared_memory_pool;
    register int i, j;
...
static void matrix_multiply_scalar(const _Float16 *a, const _Float16 *b, _Float16 *c, const int M, const int N, const int K)
{
    register int i, j, p;
