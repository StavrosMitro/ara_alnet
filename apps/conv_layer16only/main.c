...
static _Float16 act_conv1[ALEXNET_STATIC_MAX_BATCH * CONV1_OUT_UNITS];

// Vectorized: the scalar loop cost ~1108 cycles for 64 _Float16s (the build uses
...
static void zero_f16(_Float16 *buf, int n)
{
    ...
    while (remaining > 0) {
        ...
        asm volatile("vse16.v v16, (%0)" :: "r"(buf));
        buf += vl;
        remaining -= vl;
    }
}
...
void metrics(_Float16 *ret, int *preds, int *labels, 
                int classes, int totNum, int type)
{
    ...
    _Float16 accuracy = tmp_a * 1.0 / totNum;
...
void forward_alexnet(alexnet *net)
{
    ...
    zero_f16(net->conv1.output, net->batchsize * net->conv1.out_units);
...
static void gauss_initialization(_Float16 *p, int n, int in_units, int out_units)
{
    ...
    for (int shift = 0; shift < n; shift++)
    {
        if (phase == 0) {
            do {
                _Float16 U1 = (_Float16) rand() / RAND_MAX;
                _Float16 U2 = (_Float16) rand() / RAND_MAX;
