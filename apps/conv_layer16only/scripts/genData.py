...
# 1. Generate Weights and Biases (Float16)
# Weights: Random values between -0.1 and 0.1
# Bias: All 1.00 as requested
W = np.random.uniform(-0.1, 0.1, (INPUT_SIZE * OUTPUT_SIZE)).astype(np._Float1616)
B = np.ones(OUTPUT_SIZE, dtype=np._Float1616)

# 2. Generate Inputs (Float16)
# Random _Float16s between -1.0 and 1.0
X = np.random.uniform(-1.0, 1.0, (NUM_SAMPLES, INPUT_SIZE)).astype(np._Float1616)

# 3. Generate Labels/Predictions (Int16)
# For Cross Entropy, we need one integer class label per sample (0 to 511)
labels = np.random.randint(0, OUTPUT_SIZE, (NUM_SAMPLES,)).astype(np.int16)
...
with open(c_file_path, "w") as f:
    ...
    # Write Weights Array
    f.write(f"_Float16 fc1_weights[{INPUT_SIZE * OUTPUT_SIZE}] = {{\n")
    # Write in chunks of 8 values per line for readability
    for i in range(0, len(W), 8):
        chunk = W[i:i+8]
        # Append 'f' to ensure C compiler treats them as single-precision _Float16s
    ...
    # Write Bias Array
    f.write(f"_Float16 fc1_bias[{OUTPUT_SIZE}] = {{\n")
