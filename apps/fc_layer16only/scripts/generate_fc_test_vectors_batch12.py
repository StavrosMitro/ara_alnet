...
def main() -> None:
    ...
    w = np.random.uniform(-0.1, 0.1, size=(INPUTS * OUTPUTS,)).astype(np.float16)
    b = np.ones((OUTPUTS,), dtype=np.float16)
    labels = np.random.randint(0, OUTPUTS, size=(samples,)).astype(np.int32)
    targets = np.zeros((samples, OUTPUTS), dtype=np.float16)
    ...
    with weights_path.open("w", encoding="ascii") as f:
        f.write("// Auto-generated by scripts/generate_fc_test_vectors_batch12.py\n\n")
        write_c_array_f16(f, "fc1_weights_32", w)
        write_c_array_f16(f, "fc1_bias_32", b)
