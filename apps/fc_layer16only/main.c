...
static _Float16 act_fc1[ALEXNET_STATIC_MAX_BATCH * FC6_LAYER];
...
static void zero_f16(_Float16 *buf, int n)
{
    ...
    while (n > 0) {
        size_t vl;
        asm volatile("vsetvli %0, %1, e16, m8, ta, ma" : "=r"(vl) : "r"((size_t)n));
...
void forward_alexnet(alexnet *net)
{
    ...
    zero_f16(net->fc1.output, net->batchsize * net->fc1.out_units);
...
static void gauss_initialization(_Float16 *p, int n, int in_units, int out_units)
{
    float mean  = 0;
    float stddv = 0.01;
    ...
    for (int shift = 0; shift < n; shift++)
    {
        ...
        p[shift] = (_Float16)(mean + stddv * X);
    }
    (void)in_units; (void)out_units;
}
...
int main(void)
{
    ...
    alexnet_train_16(&net, ALEXNET_EPOCHS);
    // release_alexnet_static_memory(&net);
    #elif defined(ALEXNET_MODE_INFERENCE)
    const unsigned char *infer_bytes = img_data_32;
    if (ALEXNET_INFER_IDX >= 0) {
        ...
        infer_bytes = cifar10_data_32 + (size_t)ALEXNET_INFER_IDX * (size_t)cifar10_image_bytes;
        printf_("inference sample: CIFAR-10 idx=%d label=%d\n", ALEXNET_INFER_IDX, cifar10_labels_32[ALEXNET_INFER_IDX]);
    }
    setup_alexnet(&net, 1);
    malloc_alexnet(&net);
    alexnet_init_weights(&net);
    printf_("alexnet setup fininshed. Waiting for inference...\n");
    alexnet_inference_16(&net, infer_bytes);
    // release_alexnet_static_memory(&net);
    #else
