...
void make_image_32(image *img, int w, int h, int c)
{
    /**
     * Make image
     * 
     ...
     *      img
     * */
    // printf_("entering make_image_32\n");
    img->w = w;
    img->h = h;
    img->c = c;
    if (w * h * c > IMAGE_MAX_FLOATS) {
        printf_("Error! make_image_32 requested %d floats, max supported is %d\n", w * h * c, IMAGE_MAX_FLOATS);
        exit(1);
    }
    img->data = image_scratch[image_scratch_cursor];
    image_scratch_cursor = (image_scratch_cursor + 1) % IMAGE_SCRATCH_SLOTS;
    // printf_("malloc completed, no segmetation fault in make_image_32 function\n");
}

void free_image_32(image *img)
{
...
void horizontal_flip_32(image *im)
{
    /**
     * Data argumention : horizontal flip
     * */
    image tmp;
    make_image_32(&tmp, im->w, im->h, im->c);
...
void resize_image_32(image *im, int w, int h)
{
    ...
    image resized, part;
    make_image_32(&resized, w, h, im->c);
    make_image_32(&part,  w, im->h, im->c);
...
image load_image_32(const unsigned char *start_img, int W, int H, int channels, int is_h_flip) //we want to pass a pointer to the start of the array of each image.
{ //so that 4d array w,h,c,image and we traverse through arrays
    ...
    if (!data)
    {
        ...
        image empty;
        int fallback_c = channels ? channels : img_data_c_32;
        make_image_32(&empty, W, H, fallback_c);
    ...
    if ((h&&w) && (H!=img.h || W!=img.w)) {
        ...
        resize_image_32(&img, H, W);
    }
        
    
    if (is_h_flip) {
        // printf_("this image needs flipping\n");
        horizontal_flip_32(&img);
    }
                    ...
                    int w, int h, int c, int CLASSES )
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        int x_off = i * imagesize_floats;
        int copy_units = (imagesize_floats < FC_INPUT_UNITS) ? imagesize_floats : FC_INPUT_UNITS;

        memset(X + x_off, 0, (size_t)imagesize_floats * sizeof(float));
        memcpy(X + x_off, test_inputs_32 + idx * FC_INPUT_UNITS, (size_t)copy_units * sizeof(float));
        Y[i] = test_labels_32[idx];
    }
