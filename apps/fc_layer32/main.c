...
void free_forward_activations(alexnet *net)
{
     ...
     * Must be called after each forward pass when NOT followed by backward_alexnet_32
...
int main(void)
{
    ...
    alexnet_train_32(&net, ALEXNET_EPOCHS);
    ...
    alexnet_inference_32(&net, infer_bytes);
    // release_alexnet_static_memory(&net);
    #else
