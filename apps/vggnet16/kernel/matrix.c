...
_Float16 shared_memory_pool_32[MATRIX_TRANSPOSE_WORKSPACE_ELEMS];
...
_Float16 fmatmul_c_scratch_32[FMATMUL_MAX_M * FMATMUL_MAX_K];
                        ...
                        const int M, const int N, const int K)
{
    if (M <= 0 || N <= 0 || K <= 0)
        return;
    _Float16 *fmatmul_a_scratch = shared_memory_pool_32;
    ...
    const _Float16 *src_mk = fmatmul_c_scratch_32;
    _Float16 *dst_mk = c;
    while (remaining_mk > 0)
    {
        size_t vl = 0;
        asm volatile("vsetvli %0, %1, e16, m1, ta, ma" : "=r"(vl) : "r"(remaining_mk));
        // Plain copy-back: the result in fmatmul_c_scratch_32 IS the answer.
        // This used to vle16 the old c and vfadd into it, ACCUMULATING --
        // which disagreed with fmatmul_32() overwriting c on the unpadded
...
void matrix_multiply_fused_32(const _Float16 *a, const _Float16 *b, const _Float16 *bias,
                            ...
                            const int M, const int N, const int K)
{
    ...
    if (padded_m == (unsigned long int)M) {
        ...
        // Routing through fmatmul_c_scratch_32 and then copying M*K elements
                            ...
                            const int M, const int N, const int K)
{
    ...
    _Float16 *a_pad = shared_memory_pool_32;
    for (int m = 0; m < M; m++)
    {
                                  ...
                                  const _Float16 eps)
{
    ...
    _Float16 *tn_out = shared_memory_pool_32;
    ...
    if (mismatch_count == 0)
        printf_("matrix_multiply_tn_verify_32: OK\n");
    else
        printf_("matrix_multiply_tn_verify_32: FAIL (mismatches %lu)\n",
                (unsigned long)mismatch_count);

    return (mismatch_count == 0) ? 1 : 0;
}

void matrix_multiply_nt_deferred_32(const _Float16 *a, const _Float16 *b, _Float16 *c,
                                    const int M, const int N, const int K)
{
    if (M <= 0 || N <= 0 || K <= 0)
        return;
    ...
    for (size_t idx = 0; idx < mk_pad; idx++) fmatmul_c_scratch_32[idx] = (_Float16)0.0f;
                                  ...
                                  const _Float16 eps)
{
    ...
    if (mismatch_count == 0)
        printf_("matrix_multiply_nt_verify_32: OK\n");
    else
        printf_("matrix_multiply_nt_verify_32: FAIL (mismatches %lu)\n",
                (unsigned long)mismatch_count);
...
void matrix_transpose_32(_Float16 *x, int m, int n)
{
    ...
    _Float16 *tmp = shared_memory_pool_32;
    register int i, j;
