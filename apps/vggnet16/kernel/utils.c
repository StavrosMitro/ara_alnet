...
void memset_vectorized_zero_f32(_Float16 *dst, size_t n_elements)
{
    ...
    size_t vlmax;
    asm volatile("vsetvli %0, zero, e16, m8, ta, ma" : "=r"(vlmax));
...
void memset_vectorized_value_f32(_Float16 *dst, _Float16 value, size_t n_elements)
{
...
void memcpy_vectorized_f32(_Float16 *dst, const _Float16 *src, size_t n_elements)
{
    while (n_elements > 0) {
        size_t vl;
        asm volatile("vsetvli %0, %1, e16, m8, ta, ma" : "=r"(vl) : "r"(n_elements));
        asm volatile("vle16.v v16, (%0)" :: "r"(src) : "memory");
        asm volatile("vse16.v v16, (%0)" :: "r"(dst) : "memory");
        src += vl;
        dst += vl;
        n_elements -= vl;
...
void zero_f32_vec(_Float16 *buf, int n)
{
...
void vector_scale_f32(_Float16 *vec, _Float16 scale, int length)
{
    int n = length;
    while (n > 0) {
        size_t vl;
        asm volatile("vsetvli %0, %1, e16, m8, ta, ma" : "=r"(vl) : "r"(n));
        asm volatile("vle16.v v8, (%0)" :: "r"(vec) : "memory");
        asm volatile("vfmul.vf v8, v8, %0" :: "f"(scale));
        asm volatile("vse16.v v8, (%0)" :: "r"(vec) : "memory");
        vec += vl;
        n -= (int)vl;
    }
}

void vec_add_f32(_Float16 *dst, const _Float16 *src, size_t n_elements)
{
    while (n_elements > 0) {
        size_t vl;
        asm volatile("vsetvli %0, %1, e16, m8, ta, ma" : "=r"(vl) : "r"(n_elements));
...
void vec_affine_f32(_Float16 *dst, _Float16 a, _Float16 b, size_t n_elements)
{
    while (n_elements > 0) {
        size_t vl;
        asm volatile("vsetvli %0, %1, e16, m8, ta, ma" : "=r"(vl) : "r"(n_elements));
