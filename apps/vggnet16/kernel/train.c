...
#include "fmatmul.h"  // SET_FRM_RNE, CLEAR/READ_FFLAGS
...
extern _Float16 do1_mask[];    // after pool1
extern _Float16 do2_mask[];    // after pool2
extern _Float16 dofc_mask[];   // after bn5
...
static _Float16 d_conv1_weights[CONV1_W]; static _Float16 d_conv1_bias[C1_CHANNELS];
static _Float16 d_conv2_weights[CONV2_W]; static _Float16 d_conv2_bias[C1_CHANNELS];
static _Float16 d_conv3_weights[CONV3_W]; static _Float16 d_conv3_bias[C2_CHANNELS];
static _Float16 d_conv4_weights[CONV4_W]; static _Float16 d_conv4_bias[C2_CHANNELS];
static _Float16 d_fc1_weights[FC1_W];     static _Float16 d_fc1_bias[FC1_LAYER];
static _Float16 d_fc2_weights[FC2_W];     static _Float16 d_fc2_bias[OUT_LAYER];

static _Float16 d_bn1_gamma[C1_CHANNELS]; static _Float16 d_bn1_beta[C1_CHANNELS];
static _Float16 d_bn2_gamma[C1_CHANNELS]; static _Float16 d_bn2_beta[C1_CHANNELS];
static _Float16 d_bn3_gamma[C2_CHANNELS]; static _Float16 d_bn3_beta[C2_CHANNELS];
static _Float16 d_bn4_gamma[C2_CHANNELS]; static _Float16 d_bn4_beta[C2_CHANNELS];
static _Float16 d_bn5_gamma[FC1_LAYER];   static _Float16 d_bn5_beta[FC1_LAYER];

// --------------------------------------------------------------------------
// Momentum velocities, one per trainable parameter tensor
// --------------------------------------------------------------------------

static _Float16 v_conv1_weights[CONV1_W]; static _Float16 v_conv1_bias[C1_CHANNELS];
static _Float16 v_conv2_weights[CONV2_W]; static _Float16 v_conv2_bias[C1_CHANNELS];
static _Float16 v_conv3_weights[CONV3_W]; static _Float16 v_conv3_bias[C2_CHANNELS];
static _Float16 v_conv4_weights[CONV4_W]; static _Float16 v_conv4_bias[C2_CHANNELS];
static _Float16 v_fc1_weights[FC1_W];     static _Float16 v_fc1_bias[FC1_LAYER];
static _Float16 v_fc2_weights[FC2_W];     static _Float16 v_fc2_bias[OUT_LAYER];

static _Float16 v_bn1_gamma[C1_CHANNELS]; static _Float16 v_bn1_beta[C1_CHANNELS];
static _Float16 v_bn2_gamma[C1_CHANNELS]; static _Float16 v_bn2_beta[C1_CHANNELS];
static _Float16 v_bn3_gamma[C2_CHANNELS]; static _Float16 v_bn3_beta[C2_CHANNELS];
static _Float16 v_bn4_gamma[C2_CHANNELS]; static _Float16 v_bn4_beta[C2_CHANNELS];
static _Float16 v_bn5_gamma[FC1_LAYER];   static _Float16 v_bn5_beta[FC1_LAYER];
...
static _Float16 d_fc2_out  [ALEXNET_STATIC_MAX_BATCH * OUT_LAYER];   // loss grad at the logits
static _Float16 d_bn5_out  [ALEXNET_STATIC_MAX_BATCH * FC1_LAYER];
static _Float16 d_relu5_out[ALEXNET_STATIC_MAX_BATCH * FC1_LAYER];
static _Float16 d_fc1_out  [ALEXNET_STATIC_MAX_BATCH * FC1_LAYER];
static _Float16 d_pool2_out[ALEXNET_STATIC_MAX_BATCH * POOL2_UNITS];

static _Float16 d_bn4_out  [ALEXNET_STATIC_MAX_BATCH * BLOCK2_UNITS];
static _Float16 d_relu4_out[ALEXNET_STATIC_MAX_BATCH * BLOCK2_UNITS];
static _Float16 d_conv4_out[ALEXNET_STATIC_MAX_BATCH * BLOCK2_UNITS];
static _Float16 d_bn3_out  [ALEXNET_STATIC_MAX_BATCH * BLOCK2_UNITS];
static _Float16 d_relu3_out[ALEXNET_STATIC_MAX_BATCH * BLOCK2_UNITS];
static _Float16 d_conv3_out[ALEXNET_STATIC_MAX_BATCH * BLOCK2_UNITS];
static _Float16 d_pool1_out[ALEXNET_STATIC_MAX_BATCH * POOL1_UNITS];

static _Float16 d_bn2_out  [ALEXNET_STATIC_MAX_BATCH * BLOCK1_UNITS];
static _Float16 d_relu2_out[ALEXNET_STATIC_MAX_BATCH * BLOCK1_UNITS];
static _Float16 d_conv2_out[ALEXNET_STATIC_MAX_BATCH * BLOCK1_UNITS];
static _Float16 d_bn1_out  [ALEXNET_STATIC_MAX_BATCH * BLOCK1_UNITS];
static _Float16 d_relu1_out[ALEXNET_STATIC_MAX_BATCH * BLOCK1_UNITS];
static _Float16 d_conv1_out[ALEXNET_STATIC_MAX_BATCH * BLOCK1_UNITS];

// conv1's d_input: the gradient w.r.t. the image. Computed, then discarded.
static _Float16 d_image[ALEXNET_STATIC_MAX_BATCH * IMAGE_UNITS];

static float    train_input_buf  [ALEXNET_STATIC_MAX_BATCH * IMAGE_UNITS];  // fp32 from get_train_batch
static _Float16 train_input_f16  [ALEXNET_STATIC_MAX_BATCH * IMAGE_UNITS];  // narrowed -> net->input
...
static _Float16 test_input_f16   [ALEXNET_STATIC_MAX_BATCH * IMAGE_UNITS];
...
static void zero_f32(_Float16 *buf, int n)
{
...
// --------------------------------------------------------------------------
// FP16 dynamic loss scaling (fc_layer16only's algorithm, applied ONCE for the
// whole deep backward). No FP32 master weights: momentum carries sub-eps updates.
//   - CLEAR_FFLAGS before the loss; the seed multiply + every backward matmul
//     accumulate OF/UF into fflags.
//   - overflow  -> halve the scale, SKIP the optimizer this step (grads are inf).
//   - clean/UF  -> optimizer unscales each grad by 1/scale; grow the scale after
//     UF_DEBOUNCE_STEPS consecutive underflows or 1000 clean steps.
// --------------------------------------------------------------------------
#define MIN_LOSS_SCALE    1.0f
#define MAX_LOSS_SCALE    32768.0f        // 2^15, headroom below fp16 max 65504
#define UF_DEBOUNCE_STEPS 8

static float    g_loss_scale     = 1024.0f;
static _Float16 g_inv_loss_scale = (_Float16)(1.0f / 1024.0f);   // read by sgd_step_vec
static int      g_scale_ok_steps = 0;
static int      g_scale_uf_steps = 0;

// fp32 scratch for the softmax gradient before it is narrowed to fp16.
static float ce_grad_f32[ALEXNET_STATIC_MAX_BATCH * OUT_LAYER];

// Vector narrow fp32 -> fp16 (vfncvt.f.f.w): the loss/softmax run in fp32 for
// exp precision, but the backward pass consumes fp16, so the scaled gradient
// seed is narrowed here.
static void narrow_f32_to_f16_vec(_Float16 *dst, const float *src, int n)
{
    while (n > 0) {
        size_t vl = __riscv_vsetvl_e16m4((size_t)n);
        vfloat32m8_t x = __riscv_vle32_v_f32m8(src, vl);
        vfloat16m4_t y = __riscv_vfncvt_f_f_w_f16m4(x, vl);
        __riscv_vse16_v_f16m4(dst, y, vl);
...
static int fp16_grads_ok_and_adjust(void)
{
    ...
    if (flags & FFLAG_OVERFLOW_MASK) {
        g_loss_scale *= 0.5f;
        if (g_loss_scale < MIN_LOSS_SCALE) g_loss_scale = MIN_LOSS_SCALE;
        g_inv_loss_scale = (_Float16)(1.0f / g_loss_scale);
        g_scale_ok_steps = 0; g_scale_uf_steps = 0;
        return 0;
    }
    // Usable: the optimizer unscales with the scale that was actually applied.
    g_inv_loss_scale = (_Float16)(1.0f / g_loss_scale);
    if (flags & FFLAG_UNDERFLOW_MASK) {
        g_scale_ok_steps = 0;
        if (++g_scale_uf_steps >= UF_DEBOUNCE_STEPS) {
            g_loss_scale *= 2.0f;
            if (g_loss_scale > MAX_LOSS_SCALE) g_loss_scale = MAX_LOSS_SCALE;
            g_scale_uf_steps = 0;
        }
    } else {
        g_scale_uf_steps = 0;
        if (++g_scale_ok_steps > 1000) {
            g_loss_scale *= 2.0f;
            if (g_loss_scale > MAX_LOSS_SCALE) g_loss_scale = MAX_LOSS_SCALE;
            g_scale_ok_steps = 0;
        }
...
static float cross_entropy_loss(_Float16 *delta_preds, const _Float16 *preds,
                                const int *labels, int units, int BATCH_SIZE)
{
    // Softmax + cross-entropy in FP32 (exp has no fp16 form). Logits arrive in
    // fp16 and are widened per element; the true (unscaled) loss is returned for
    // reporting. The gradient (softmax - onehot) is scaled by loss_scale/B in
    // fp32, then narrowed to fp16 for the backward pass.
    float total_loss = 0.0f;
    const float seed_scale = g_loss_scale / (float)BATCH_SIZE;

    for (int p = 0; p < BATCH_SIZE; ++p) {
        const _Float16 *pp = preds + (size_t)p * units;
        float          *gp = ce_grad_f32 + (size_t)p * units;

        float maxv = (float)pp[0];
        for (int i = 1; i < units; ++i) { float v = (float)pp[i]; if (v > maxv) maxv = v; }

        float esum = 0.0f;
        for (int i = 0; i < units; ++i) esum += expf((float)pp[i] - maxv);

        float tp = expf((float)pp[labels[p]] - maxv) / esum;
        if (tp < 1e-7f) tp = 1e-7f;
        total_loss -= logf(tp);

        for (int i = 0; i < units; ++i) {
            float sm = expf((float)pp[i] - maxv) / esum;
            gp[i] = (sm - (i == labels[p] ? 1.0f : 0.0f)) * seed_scale;
        }
    }
...
static void bw_trace(const char *name, const _Float16 *p, int n);
#endif

// Scalar reference (build -DSGD_SCALAR). Unscales the loss-scaled gradient,
// applies weight decay, momentum, [-1,1] clip. FP16 throughout, no master copy.
static void sgd_step_scalar(_Float16 *w, _Float16 *v, const _Float16 *dw, int n, _Float16 wd, _Float16 lr)
{
    for (int i = 0; i < n; i++) {
        _Float16 g  = dw[i] * g_inv_loss_scale;                     // true gradient
        _Float16 vi = (_Float16)MOMENTUM * v[i] - lr * (g + wd * w[i]);
        if (vi >  (_Float16)1.0f) vi =  (_Float16)1.0f;
        if (vi < (_Float16)-1.0f) vi = (_Float16)-1.0f;
        v[i]  = vi;
        w[i] += vi;
    }
...
// Vectorized FP16 momentum SGD (structural mirror of fc_layer16only's
// momentum_sgd_vec_16). First unscales the loss-scaled gradient by
// g_inv_loss_scale, then dw + wd*w, then MOMENTUM*v - lr*t, clip, w += v.
static void sgd_step_vec(_Float16 *w, _Float16 *v, const _Float16 *dw, int n, _Float16 wd, _Float16 lr)
{
    const _Float16 momentum  = (_Float16)MOMENTUM;
    const _Float16 clip_lo   = (_Float16)-1.0f;
    const _Float16 clip_hi   = (_Float16)1.0f;
    const _Float16 inv_scale = g_inv_loss_scale;

    while (n > 0) {
        size_t vl;
        asm volatile("vsetvli %0, %1, e16, m8, ta, ma" : "=r"(vl) : "r"(n));

        asm volatile("vle16.v v8,  (%0)" :: "r"(v)  : "memory");   // velocity
        asm volatile("vle16.v v16, (%0)" :: "r"(dw) : "memory");   // scaled gradient
        asm volatile("vle16.v v24, (%0)" :: "r"(w)  : "memory");   // weights

        asm volatile("vfmul.vf  v16, v16, %0" :: "f"(inv_scale));  // -> true gradient
        asm volatile("vfmacc.vf v16, %0, v24" :: "f"(wd));         // + wd*w
        asm volatile("vfmul.vf   v8, v8, %0"  :: "f"(momentum));   // MOMENTUM*v
        asm volatile("vfnmsac.vf v8, %0, v16" :: "f"(lr));         // - lr*t
        asm volatile("vfmax.vf v8, v8, %0" :: "f"(clip_lo));
        asm volatile("vfmin.vf v8, v8, %0" :: "f"(clip_hi));
        asm volatile("vfadd.vv v24, v24, v8");                     // w += v

        asm volatile("vse16.v v8,  (%0)" :: "r"(v) : "memory");
        asm volatile("vse16.v v24, (%0)" :: "r"(w) : "memory");

        w += vl; v += vl; dw += vl; n -= (int)vl;
    }
}

static inline void sgd_step(_Float16 *w, _Float16 *v, const _Float16 *dw, int n, _Float16 wd, _Float16 lr)
{
#ifdef SGD_SCALAR
...
float backward_alexnet(alexnet *net, int *batch_Y)
{
    ...
    // Arm the FP16 overflow/underflow detector for this whole step.
    CLEAR_FFLAGS();
    ...
    if (fp16_grads_ok_and_adjust()) gradient_descent(net);
    unbind_d_params(net);
#ifdef DUMP_TRACE
    bw_trace_done = 1;
#endif
...
void alexnet_train(alexnet *net, int epochs)
{
    ...
    for (int e = 0; e < epochs; e++) {
        ...
        for (int b = 0; b < steps_per_epoch; b++) {
            ...
            // so a global input scale is neutralised past the first BN -- it
            // only shifts conv1's raw activation range. Kept as an experiment
            // knob; default (unset) = 1.0 = off.
            vector_scale_f32(net->input, (_Float16)(INPUT_SCALE),
                             net->batchsize * IMAGE_UNITS);
#endif
...
void alexnet_test(alexnet *net)
{
    ...
    for (int b = 0; b < steps; b++) {
        ...
        int valid = get_eval_batch(net->batchsize, test_input_buf, batch_Y,
